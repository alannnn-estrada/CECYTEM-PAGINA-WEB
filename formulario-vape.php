...
// ... (Código previo y conexión a la base de datos utilizando mysqli)

// Consulta preparada para buscar el id_usuario en la tabla formulario
$query = "SELECT id_usuario FROM formulario WHERE id_usuario = ?";
...
if (!empty($id_usuario)) {
    // Hay registros en la tabla formulario para el id_usuario
    $formU = true;
} else {
    // No se encontraron registros en la tabla formulario para el id_usuario
    $formU = false;
}

?>
<?php if($rol == 'Admin' || $rol == 'Estudiante' && $formU == 'false' ):?>
    ...
    .resultados{
        ...
        <form class="mt-1" method="POST" <?php if(isset($correctas) && $correctas != 7) { echo "disabled"; } ?>>
...
<?php else:?>
    <?php include('head-nav.php')?>
    <center><h1>Ya has contestado este formulario, solo se puede responder una vez por persona!</h1></center>
